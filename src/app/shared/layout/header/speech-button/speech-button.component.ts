import {Component, OnInit, AfterContentInit, ContentChild, ViewChild} from '@angular/core';
...
import {SpeechAppModel} from "./speech-app.model";
...
export class SpeechButtonComponent implements OnInit, AfterContentInit {


  @ViewChild('popover') popover;
  @ViewChild('speechBtn') speechBtn;

  public hasSpeechRecognition: boolean = false;
  ...
  ngOnInit() {
    ...
    root['smartSpeechRecognition'] = {
      ...
      init: function (commands, resetCommands) {
        ...
        recognition.onstart = function () {
          ...
          if (config.debugState) {
            root.console.log('%c ✔ SUCCESS: User allowed access the microphone service to start ', config.debugStyle_success);
            root.console.log('Language setting is set to: ' + recognition.lang, config.debugStyle);
          }
        ...
        recognition.onerror = function (event) {
          ...
          switch (event.error) {
            ...
            case 'service-not-allowed':
              ...
              //debugState
              if (config.debugState) {
                root.console.log('%c WARNING: Microphone was not detected (either user denied access or it is not installed properly) ', config.debugStyle_warning);
              }
        ...
        recognition.onresult = function (event) {
          ...
          for (var i = 0; i < results.length; i++) {
            ...
            if (config.debugState) {
              root.console.log('Speech recognized: %c' + commandText, config.debugStyle);
            }
            ...
            for (var j = 0, l = commandsList.length; j < l; j++) {
              ...
              if (result) {
                ...
                if (config.debugState) {
                  root.console.log('command matched: %c' + commandsList[j].originalPhrase, config.debugStyle);
      ...
      addCommands: function (commands) {
        ...
        if (config.debugState) {
          root.console.log('Commands successfully loaded: %c' + commandsList.length, config.debugStyle);
        }
      },
  ...
  ngAfterContentInit(){
    let $element = $(this.speechBtn.nativeElement);
    let $popover = $(this.popover.nativeElement);
    if (this.hasSpeechRecognition && config.voice_command) {
